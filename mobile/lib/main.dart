...
// Configuración de la dirección del servidor Backend
class ApiConfig {
  static String serverIp = "10.0.2.2"; // Cambiar a la IP de tu PC para probar en físico
  static String port = "8000";
  static String? token; // Token JWT para autenticación
  static String? userLogged;

  static String get baseUrl => "http://$serverIp:$port";
}

// Controlador de Pestañas (Monitoreo, Configuración/Login)
class MainTabController extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      length: 2, // Reducido a solo 2 pestañas clave para simplificar la exposición
      child: Scaffold(
        appBar: AppBar(
          title: const Text('EcoPulse - Calidad del Aire'),
          ...
          bottom: const TabBar(
            ...
            tabs: [
              ...
              Tab(icon: Icon(Icons.settings), text: "Configuración / Auth"),
            ],
          ),
        ),
...
// ==========================================
// 1. PANTALLA PRINCIPAL: MONITOREO EN TIEMPO REAL
// ==========================================
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Future<void> _fetchData() async {
    ...
    try {
      // Consumimos el endpoint público del backend
  ...
  // Define el color de alerta basado en el nivel de PM2.5
  Color _getSeverityColor(double pm25) {
    if (pm25 <= 12) return Colors.green; // Bueno
    if (pm25 <= 35) return Colors.orange; // Moderado
...
// ==========================================
// 2. PANTALLA: CONFIGURACIÓN, REGISTRO Y LOGIN
// ==========================================
...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Future<void> _submitAuth() async {
    ...
    if (username.isEmpty || password.isEmpty) {
      setState(() {
        _authMessage = "Completa todos los campos";
      });
      return;
    }
    ...
    try {
      if (_isRegister) {
        // Petición de registro en tu FastAPI
        ...
        if (response.statusCode == 200) {
          setState(() {
            _authMessage = "¡Registro exitoso! Ya puedes iniciar sesión.";
            _isRegister = false;
          });
        } else {
          final err = jsonDecode(response.body);
          setState(() {
            _authMessage = "Error: ${err['detail'] ?? 'No se pudo registrar'}";
          });
        }
      } else {
        // Login OAuth2 que configuraste en backend
        final response = await http.post(
        ...
        if (response.statusCode == 200) {
          ...
          setState(() {
            _authMessage = "Credenciales incorrectas.";
          });
        }
      }
    } catch (e) {
      setState(() {
        _authMessage = "Error de conexión con el servidor.";
      });
    }
  }
          ...
          children: [
            ...
            if (logged) ...[
              ...
              ElevatedButton(
                onPressed: () {
                  setState(() {
                    ...
                    ApiConfig.userLogged = null;
                    _authMessage = "Sesión cerrada";
                  });
                },
              ...
              if (_authMessage.isNotEmpty)
                Text(_authMessage, style: const TextStyle(color: Colors.blue, fontWeight: FontWeight.bold)),
              const SizedBox(height: 12),
              ElevatedButton(
                onPressed: _submitAuth,
                style: ElevatedButton.styleFrom(backgroundColor: Colors.green),
                child: Text(_isRegister ? "Registrarse" : "Entrar", style: const TextStyle(color: Colors.white)),
              ),
              TextButton(
                onPressed: () {
                  setState(() {
